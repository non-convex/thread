...
内容层次主要靠文字明度和少量协调的色相区分。回合与 Markdown 标题使用更鲜明的薄荷色，工具名和页脚模型名保留原来的橙色，使用独立的 `nameAccent`，不影响其他薄荷色强调；工具名保留粗体以便扫描，模型名使用常规字重。正文保持中性色，不把整段回答染绿。工具调用信息（命令、路径、参数）的视觉强度介于主正文和次要正文之间。普通工具输出展开后使用次要正文色，折叠预览和耗时继续减弱，让调用信息与结果保持层次。

| 用途 | 色彩角色 | 暗色 | 亮色 |
| --- | --- | --- | --- |
| 回合标题、Markdown 标题、欢迎标志 | `accentStrong` | `#ADE6CF` | `#165E4B` |
| 链接、操作强调 | `accent` | `#80CCB2` | `#26765F` |
| 工具名、页脚模型名 | `nameAccent` | `#C8936D` | `#EA580C` |
| 内联代码、欢迎页命令提示 | `accentDim` | `#70A996` | `#4A7766` |
...
| 进行中的 spinner、忙碌边框 | `spark` | `#83C4D4` | `#2E7385` |
