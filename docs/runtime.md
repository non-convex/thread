...
`webfetch` 不自动跟随 HTTP 重定向。收到跳转响应时，工具返回目标 URL，但不会访问它；需要继续时，应以该 URL 发起一次新的 `webfetch` 调用。这样，每个实际访问的地址都先经过正常的宿主工具授权。
...
实际写入先在目标所在目录准备一个临时文件，写完并同步后，再次检查目标路径与文件版本，最后替换目标。临时文件写入失败或在替换前取消，不会截断原文件；已有文件的权限位（mode）会保留。Windows 上还会在写入临时文件前，通过系统 PowerShell 复制原文件的访问控制列表（DACL）；复制失败时拒绝替换，不静默放宽权限。创建新文件使用独占发布，若另一个写入者先创建了目标，本次创建会失败，不覆盖对方的文件。

替换只改变获准路径所对应的文件，不原位改写它与其他路径共享的内容。因此，目标原本具有硬链接时，其他链接仍保留原内容。路径和版本检查仍不是针对外部进程的文件系统沙箱。
...
不超过 1 MiB 的文件沿用完整读取缓冲区计算 SHA-256；更大的文件保持范围读取，只记录文件身份、大小、修改时间和状态变更时间，避免为了版本检查额外扫描整个大文件。读取前后会检查版本稳定性；实际写入在已有的同路径队列内校验，并在 checkpoint 保存后、临时文件准备好且即将替换目标时再次检查路径与版本。如果在最后这次检查才发现变化，写入仍会拒绝，但可能已经留下本轮的备份记录。关闭 checkpoint 不关闭这些检查。
...
如果恢复失败，runtime 会报告未完成的回退，并停止接收新的前台操作。排除权限、磁盘空间等问题后，关闭并重新打开项目，启动过程会根据已记录的目标重新恢复所有相关路径，再开放执行。即使重新打开时关闭了 checkpoint，也会先完成此前已开始的回退。不要在等待恢复期间继续修改这些文件，因为恢复会再次覆盖它们。
