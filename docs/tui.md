...
`SessionScreen` 是一张相对定位的整屏：上方是 transcript 或欢迎页，下方依次是常驻 Worker 卡片栏（有 Worker 时）、目标行（有目标时）、执行状态行、输入框和页脚。浮层位于这组控件之上；目标行显示时增加一行高度，transcript 和浮层的位置随之调整。输入框是 `surfaceHigh` 底的圆角框，忙碌时边框变成 `runningAccent`。输入区高度随内容在 1–4 行之间变化，transcript 的底边跟着让。
...
有 Worker 时，状态区上方显示当前 live turn 的常驻卡片栏，卡片栏下方留一行空白；回合完成后改读当前 live tip 对应 turn 的历史，直到下一轮开始。切换 Session、rewind 和 `/clear` 时随当前内容更新。Transcript 中原有的一行 Worker 任务块继续作为历史保留。
...
plain 模式也会输出目标状态。交互式 plain 在工作输入运行时继续接受查看、暂停和清除命令；非交互式 plain 则先等待工作输入结束，再读取下一行，EOF 也会等待已接受的输入完成。

## 状态与页脚

执行状态行在输入框上方。当前 Session 有目标时，在执行状态行上方单独显示一行目标信息，左边与执行信息对齐，格式为 `goal active · turn 1 · 目标内容`。这里只显示当前轮次，不显示轮数上限；目标文本按可用行宽截断，没有目标时不占行。切换会话、启动和 rewind 后从持久状态读取，详细内容使用 `/goal status`。

执行状态行忙碌时左侧依次显示共享时钟的 braille spinner、活动文案和耗时；耗时紧跟在活动文案后面，仍属于左侧信息区，右侧只提示 `esc interrupt`。空间不足时先截断活动文案，优先保留计时。文案来自当前 activity（thinking / 工具名 / compacting / workers），结束后留下 `worked <duration>` 或 notice，notice 附带的独立耗时同样跟在文案后面。
...
打开模型、Agent 等菜单只更新浮层，不重新读取会话或计算历史预览。会话切换、回退、回合结束和成功的手动压缩才刷新聊天记录；每个 `turn_finished` 是持久回合的交接点（多轮目标逐轮交接），输入返回后只兜底尚未交接的回合或异常，不重复投影历史与计算上下文。外部 runtime 发起的回合也在结束事件中刷新并结束 busy，不把当前运行中的回合读入历史。切换模型或 Agent 设置仍更新相关上下文统计，`/clear` 不会被命令收尾重新加载的历史覆盖。
