...
export function fileEditingPrompt(fileCheckpoints = true): string {
  return fileCheckpoints ? FILE_EDITING_PROMPT : `Prefer the built-in edit tool for partial file changes and write for creating or replacing files. Use bash for commands that require it, such as builds and tests; do not use shell redirection, sed, or scripts in place of edit/write for ordinary file edits. File checkpoints are disabled: file changes are not backed up, and conversation rewind leaves files unchanged.`;
}
