...
Codex 设计复查后的落实项：worker 的 `writeScope` 在现有内置文件写入入口强制检查；coding 应用启动时读取根目录 `AGENTS.md`，通过 `sharedInstructions` 共享给主 agent 和 worker；仓库增加精简指引，CI 执行类型检查、静态依赖边界检查和构建；独立宿主示例按需手动运行。继续复用现有执行器和文件服务，没有增加沙箱框架、分层指令扫描或新的协议层。
...
嵌入示例应使用包的公开导出，避免隐藏的 TUI preload 或私有入口掩盖嵌入问题。
...
后续 runtime 实现以“现有 TUI + 一个独立嵌入示例共用公共 API”为完成条件。离线示例使用脚本模型展示工具执行与公开包用法，不代表真实模型提供方、网络工具服务或尚未实现的 MCP 已通过验证。
