...
内容层次主要靠文字明度和少量协调的色相区分。回合与 Markdown 标题使用更鲜明的薄荷色，工具名和模型名低一档；正文保持中性色，不把整段回答染绿。参数、预览和耗时依次减弱，主动展开的普通工具输出则提高到次要正文的亮度。

| 用途 | 色彩角色 | 暗色 | 亮色 |
| --- | --- | --- | --- |
| 回合标题、Markdown 标题、欢迎标志 | `accentStrong` | `#ADE6CF` | `#165E4B` |
| 工具名、模型名、链接 | `accent` | `#80CCB2` | `#26765F` |
| 内联代码、欢迎页命令提示 | `accentDim` | `#70A996` | `#4A7766` |
| 主正文 | `text` | `#C0CACF` | `#2B3B3E` |
| 参数、引用、展开的普通工具输出 | `softText` | `#98A7AE` | `#4F626A` |
