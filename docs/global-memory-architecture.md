...
Thread 使用一个轻量的跨项目记忆文件保存长期稳定且与单一项目无关的信息。Main 负责用户明确要求记住的跨项目信息；Dreamer 则从长期互动和 Agent 工作轨迹中发现未被直接说明、但有充分证据支持的用户模式与可迁移经验。实现不引入数据库、记忆版本、审批流、结构化记忆模型、专用记忆工具或跨进程锁。
...
文件最多保留 15 条带时间戳的 Markdown 列表项。Main 的提示词要求使用 `- [YYYY-MM-DD] memory content` 格式，每条保存一个独立有用的事实或规则；新增或修订条目使用当前日期，未修改条目保留原日期。维护时合并重复项、更新被新信息取代的条目，并保留其他有价值的内容；必要时移除过时或价值较低的条目以保持数量限制。文件不存在等价于空记忆。
...
每个结束的前台 turn 会作为一个独立单元加入待审阅队列；队列只保存 turn ID，不复制原始消息或图片。累计 10 个结束 turn 后，调度器从 Main 进入空闲状态的时刻开始计时；只有 Main 连续空闲 10 分钟才会启动 Dreamer。Compaction 不再单独触发 Dreamer，也不会把被压缩消息重复加入待审阅队列。

审阅时通过只读 `readTurn(turnId): Iterable<Message>` 按需从 Session Tree 借用消息，逐 turn 格式化并按完整 turn 贪心装入当前批次；不会先组装所有待处理 turn 的全文。每批审阅消息最多占 Dreamer 模型上下文窗口的 50%；单个 turn 本身超限时，以首尾内容及省略标记缩略。一次启动的所有批次共享同一个 5 分钟总时限和待处理 ID 快照；运行期间新加入的 turn 留给下一轮。

新用户输入会取消尚未到期的空闲计时，但不会中止已经运行的 Dreamer；两者可以并行。读取和组批会合作式让出事件循环并检查取消信号。每个批次成功后只移除该批覆盖的完整 turn ID；当前批次失败或总时限到期时，当前批次和后续批次仍保留，并设置独立的重试延迟。关闭或禁用 Dreamer 才会取消运行并清除待审阅内容。
