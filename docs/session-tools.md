...
`details` 同时提供 `offset`、`shownBytes`、`totalBytes`，还有未读内容时提供 `nextOffset`。这些位置针对按本次选项格式化后的整段历史，不包含分页提示。最后一页没有 `nextOffset`。短历史仍按原来的格式完整返回；原始记录不会因分页被截断或修改。读取会先分段扫描计算 UTF-8 总长度，再按字节偏移提取一页，不预先拼接或编码整段历史；因此续读须保持所有选项及原始记录不变。当前没有按 `entryId` 单独读取或自定义每页长度的参数。
