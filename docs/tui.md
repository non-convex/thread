...
diffAdded   #6FA377    工具 diff 的新增行，低饱和绿
diffRemoved #B47A72    工具 diff 的删除行，低饱和红
...
失败原因直接显示，长诊断仍受预览高度限制。取消和权限拒绝不会显示成成功或普通执行失败。预览按终端列宽计算，`edit`、`write` 最多十个屏幕行，其余工具仍最多五行；中文、emoji 或单行长 JSON 不会绕过对应的行数上限。输出继续使用无边框、无底色的缩进布局。Diff 的折叠预览和展开正文使用同一套低饱和灰绿／灰红配色，不复用更醒目的成功／错误状态色；长行换行后仍保留原行的增删颜色。
