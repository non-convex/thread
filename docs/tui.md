...
点击卡片打开卡片栏上方的 Worker 详情浮层，再点同一卡片关闭；卡片栏保持可见。详情只有一条连续的对话流，不再分成 Prompt 和 Trace 页签。初始任务以 `Main agent` 消息开始，沿用主界面用户消息的底色带，并提供复制按钮；后面依次展示 Worker 的思考、工具调用和回复。主 agent 要求返工时，反馈作为下一条 `Main agent` 消息追加，已有对话不会清空。
